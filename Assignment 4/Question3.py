...
"""Apply Gaussian high pass filter to Flowers.jpg accompanying this assignment, and discuss
your results and the effect of cutoff frequency."""
...
[u, v] = np.meshgrid(x, y)
r_sq = u**2 + v**2
#ghpf = 1 - np.exp((-r_sq) / (2 * (r0 ** 2)))
ghpf = 1 - np.exp((-r_sq) / (2 * (30**2)))

#Applying FFT on the images
padded_img_fft = np.fft.fftshift(np.fft.fft2(padded_img))
padded_img_filtered = padded_img_fft * ghpf
...
ax[0][1].imshow(ghpf, cmap = plt.cm.gray)
